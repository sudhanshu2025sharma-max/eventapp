...
function Shimmer({ w, h, radius = RADIUS.sm, style }) {
  ...
  useEffect(() => {
    Animated.loop(Animated.timing(anim, { toValue: 1, duration: 1200, easing: Easing.inOut(Easing.ease), useNativeDriver: true })).start();
  }, []);
  ...
  return (
    <View style={[s.card, { padding: SPACE.lg }]}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACE.md, marginTop: SPACE.lg }}>
        ...
        <View style={{ flex: 1, gap: SPACE.xs }}><Shimmer w={W * 0.3} h={10} /><Shimmer w={W * 0.2} h={8} /></View>
      </View>
...
function ReactionPill({ r, count, active, onPress }) {
  ...
  const tap = () => {
    Animated.sequence([Animated.spring(scale, { toValue: 1.35, tension: 400, friction: 6, useNativeDriver: true }), Animated.spring(scale, { toValue: 1, tension: 250, friction: 8, useNativeDriver: true })]).start();
    onPress();
  };
  return (
    <TouchableOpacity onPress={tap} activeOpacity={0.75}>
      <Animated.View style={[s.pill, active && s.pillActive, { transform: [{ scale }] }]}>
  ...
  return (
    <TouchableOpacity activeOpacity={0.7} style={s.sessionWrap}>
      <View style={s.sessionInner}>
        <View style={s.sessionIcon}><Ionicons name="calendar-outline" size={14} color={COLORS.brand} /></View>
...
const PostCard = React.memo(({ item, index, onReact, onOpenComments, onOpenMedia }) => {
  ...
  const isVideo = item.image_url && item.image_url.match(/\.(mp4|mov|m4v)$/i);
  const totalReactions = useMemo(() => Object.values(item.reaction_summary || {}).reduce((a, b) => a + b, 0), [item.reaction_summary]);
  const topEmojis = useMemo(() => Object.entries(item.reaction_summary || {}).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([k]) => REACTIONS.find(r => r.key === k)?.emoji).filter(Boolean), [item.reaction_summary]);
    ...
    <Animated.View style={{ opacity: fadeIn, transform: [{ translateY: slideUp }], marginBottom: SPACE.lg }}>
      <View style={s.card}>
        <LinearGradient colors={meta.colors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.ribbon}>
          <View style={s.ribbonLeft}><View style={s.ribbonIcon}><Ionicons name={meta.icon} size={12} color={COLORS.textInverse} /></View><Text style={s.ribbonLabel}>{meta.tag}</Text></View>
          <Text style={s.ribbonTime}>{relativeTime(item.published_at)}</Text>
        </LinearGradient>

        {item.pinned && <View style={s.pinRow}><Ionicons name="pin" size={11} color={COLORS.accentDark} /><Text style={s.pinText}>Pinned</Text></View>}

        <View style={s.author}>
          <LinearGradient colors={meta.colors} style={s.avatar}><Text style={s.avatarLetter}>{(item.author_name || 'E')[0].toUpperCase()}</Text></LinearGradient>
          <View style={{ flex: 1 }}><Text style={s.authorName}>{item.author_name || 'ETD 2026'}</Text><Text style={s.authorRole}>{item.author_role || 'Organizer'}</Text></View>
        </View>

        <Text style={s.title}>{item.title}</Text>
        
        <View style={{ paddingHorizontal: SPACE.lg, marginTop: SPACE.sm }}>
          <Text style={s.body} numberOfLines={expandedText ? undefined : 3}>{item.body}</Text>
          {item.body && item.body.length > 120 && (
            <TouchableOpacity onPress={() => setExpandedText(!expandedText)}>
              <Text style={s.readMoreTxt}>{expandedText ? 'Show less' : 'Read more'}</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        {totalReactions > 0 && (
          <View style={s.summaryRow}>
            <View style={s.summaryBubbles}>{topEmojis.map((e, i) => <View key={i} style={[s.summaryBubble, i > 0 && { marginLeft: -6 }]}><Text style={{ fontSize: FONT.xs + 2 }}>{e}</Text></View>)}</View>
        ...
        <View style={s.reactRow}>
          {visibleReactions.map(r => <ReactionPill key={r.key} r={r} count={item.reaction_summary?.[r.key] || 0} active={item.my_reaction === r.key} onPress={() => onReact(item.id, r.key)} />)}
          {!expandedReactions && <TouchableOpacity onPress={() => setExpandedReactions(true)} style={s.moreBtn}><Ionicons name="ellipsis-horizontal" size={14} color={COLORS.textTer} /></TouchableOpacity>}
        </View>

        <View style={s.bottomBar}>
          <TouchableOpacity style={s.commentBtn} onPress={() => onOpenComments(item)} activeOpacity={0.7}><Ionicons name="chatbubble-outline" size={15} color={COLORS.brand} /><Text style={s.commentBtnText}>{item.comment_count || 0} {(item.comment_count || 0) === 1 ? 'Comment' : 'Comments'}</Text></TouchableOpacity>
          {!item.allow_comments && <View style={s.lockRow}><Ionicons name="lock-closed" size={10} color={COLORS.textTer} /><Text style={s.lockLabel}>Closed</Text></View>}
        </View>
      </View>
    </Animated.View>
  );
...
function CommentItem({ item, onReply, idx }) {
  ...
  return (
    <Animated.View style={[s.cCard, { opacity: fadeIn, transform: [{ translateX: slideX }] }]}>
      <View style={s.cRow}>
        <View style={s.cAvatar}><Text style={s.cAvatarLetter}>{(item.user_name || 'U')[0].toUpperCase()}</Text></View>
        <View style={{ flex: 1 }}>
          <View style={s.cMeta}><Text style={s.cName}>{item.user_name}</Text><Text style={s.cTime}>{relativeTime(item.created_at)}</Text></View>
          <Text style={s.cBody}>{item.body}</Text>
          <TouchableOpacity onPress={() => onReply(item)} style={s.cReplyBtn} activeOpacity={0.7}><Ionicons name="arrow-undo-outline" size={12} color={COLORS.brand} /><Text style={s.cReplyText}>Reply</Text></TouchableOpacity>
        </View>
      </View>
      {item.replies?.map(r => (
        <View key={r.id} style={s.cReplyCard}>
          ...
          <View style={{ flex: 1 }}>
            <View style={s.cMeta}><Text style={s.cName}>{r.user_name}</Text><Text style={s.cTime}>{relativeTime(r.created_at)}</Text></View>
...
export default function FeedScreen({ onBack }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    } catch { if (!silent) Alert.alert('Error', 'Could not load feed.'); } 
  ...
  const react = async (postId, type) => {
    ...
    try {
      const res = await apiFetch(`/posts/feed/${postId}/react/`, { method: 'POST', body: JSON.stringify({ reaction_type: removing ? '' : type }) });
      const data = await res.json();
      if (!res.ok) throw new Error();
      setPosts(ps => ps.map(p => p.id === postId ? { ...p, reaction_summary: data.summary || p.reaction_summary, my_reaction: data.my_reaction ?? null } : p ));
    } catch { load(true); }
  };

  const openComments = async (post) => {
    setActivePost(post); setReplyTo(null); setText(''); setModalOpen(true); setCommentsLoading(true);
    Animated.parallel([
  ...
  const submitComment = async () => {
    ...
    setSending(true);
    try {
      const res = await apiFetch(`/posts/feed/${activePost.id}/comments/`, { method: 'POST', body: JSON.stringify({ body, parent_id: replyTo?.id || null }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.detail || 'Failed');
      if (replyTo?.id) setComments(cs => cs.map(c => c.id === replyTo.id ? { ...c, replies: [...(c.replies || []), data.comment] } : c));
      else setComments(cs => [...cs, { ...data.comment, replies: data.comment.replies || [] }]);
      setPosts(ps => ps.map(p => p.id === activePost.id ? { ...p, comment_count: (p.comment_count || 0) + 1 } : p));
      setText(''); setReplyTo(null);
    } catch (e) { Alert.alert('Error', e?.message || 'Could not post comment.'); }
    finally { setSending(false); }
  };

  const headerBorder = scrollY.interpolate({ inputRange: [0, 60], outputRange: [0, 1], extrapolate: 'clamp' });

  if (loading) {
    return (
      <View style={s.screen}>
        <View style={s.header}><TouchableOpacity onPress={onBack} style={s.headerIcon}><Ionicons name="arrow-back" size={20} color={COLORS.text} /></TouchableOpacity><View style={{ flex: 1 }}><Text style={s.headerTitle}>Feed</Text></View></View>
        <View style={{ padding: SPACE.lg, gap: SPACE.lg }}><SkeletonCard /><SkeletonCard /></View>
      </View>
    );
  }
  ...
  return (
    <View style={s.screen}>
      <Animated.View style={[s.header, { borderBottomColor: headerBorder.interpolate({ inputRange: [0, 1], outputRange: ['transparent', COLORS.border] }) }]}>
        <TouchableOpacity onPress={onBack} style={s.headerIcon} activeOpacity={0.7}><Ionicons name="arrow-back" size={20} color={COLORS.text} /></TouchableOpacity>
        <View style={{ flex: 1 }}><Text style={s.headerTitle}>Feed</Text><Text style={s.headerSub}>Updates & announcements</Text></View>
        <TouchableOpacity onPress={refresh} style={s.headerIcon} activeOpacity={0.7}><Ionicons name="refresh" size={18} color={COLORS.brand} /></TouchableOpacity>
      </Animated.View>
      ...
      <Animated.FlatList
        ...
        renderItem={({ item, index }) => <PostCard item={item} index={index} onReact={react} onOpenComments={openComments} onOpenMedia={(url, isVideo) => setFullMedia({url, isVideo})} />}
        ListEmptyComponent={
          <View style={s.empty}>
            <View style={s.emptyCircle}><Ionicons name="newspaper-outline" size={40} color={COLORS.brand} /></View>
            <Text style={s.emptyTitle}>No posts yet</Text>
            <Text style={s.emptySub}>Conference updates will appear here</Text>
            <TouchableOpacity onPress={refresh} style={s.emptyBtn} activeOpacity={0.7}><Text style={s.emptyBtnText}>Refresh</Text></TouchableOpacity>
          </View>
        }
      />

      <Modal visible={!!fullMedia} transparent animationType="fade" onRequestClose={() => setFullMedia(null)}>
        <View style={s.mediaViewerContainer}>
          <TouchableOpacity style={s.mediaCloseBtn} onPress={() => setFullMedia(null)}>
      ...
      <Modal visible={modalOpen} animationType="none" transparent onRequestClose={closeComments}>
        <View style={s.modalWrap}>
          <Animated.View style={[StyleSheet.absoluteFill, { backgroundColor: COLORS.brandDeeper, opacity: overlay.interpolate({ inputRange: [0, 1], outputRange: [0, 0.45] }) }]}><TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={closeComments} /></Animated.View>
          <Animated.View style={[s.sheet, { transform: [{ translateY: sheetY }] }]}>
            <View style={s.sheetHandleWrap}><View style={s.sheetHandle} /></View>
            <View style={s.sheetHeader}>
              <View style={s.sheetTitleRow}><Ionicons name="chatbubbles-outline" size={18} color={COLORS.brand} /><Text style={s.sheetTitle}>Comments</Text>{comments.length > 0 && <View style={s.badge}><Text style={s.badgeText}>{comments.length}</Text></View>}</View>
              <TouchableOpacity onPress={closeComments} style={s.sheetClose} activeOpacity={0.7}><Ionicons name="close" size={18} color={COLORS.textSec} /></TouchableOpacity>
            </View>
            {activePost && !activePost.allow_comments && <View style={s.closedBanner}><Ionicons name="lock-closed-outline" size={13} color={COLORS.accentDark} /><Text style={s.closedBannerText}>Comments are disabled</Text></View>}
            {commentsLoading ? (
              <View style={s.sheetLoading}><ActivityIndicator size="small" color={COLORS.brand} /><Text style={s.sheetLoadingText}>Loading…</Text></View>
            ) : (
              <ScrollView contentContainerStyle={{ padding: SPACE.lg, paddingBottom: keyboardHeight > 0 ? keyboardHeight + 80 : 160 }} showsVerticalScrollIndicator={false}>
                {comments.length ? comments.map((c, i) => <CommentItem key={c.id} item={c} onReply={setReplyTo} idx={i} />) : <View style={s.emptyComments}><View style={s.emptyCommentsCircle}><Ionicons name="chatbubble-ellipses-outline" size={28} color={COLORS.brand} /></View><Text style={s.emptyCommentsTitle}>No comments yet</Text><Text style={s.emptyCommentsSub}>Be the first to share your thoughts</Text></View>}
              </ScrollView>
            )}
            {activePost?.allow_comments && (
              <View style={[s.inputBar, { marginBottom: keyboardHeight }]}>
                  {replyTo && <View style={s.replyBar}><View style={s.replyBarLeft}><Ionicons name="arrow-undo-outline" size={12} color={COLORS.brand} /><Text style={s.replyBarText} numberOfLines={1}>Replying to <Text style={{ fontWeight: FONT.w7 }}>{replyTo.user_name}</Text></Text></View><TouchableOpacity onPress={() => setReplyTo(null)}><Ionicons name="close-circle" size={16} color={COLORS.textTer} /></TouchableOpacity></View>}
                  <View style={s.inputRow}>
                    <TextInput value={text} onChangeText={setText} placeholder={replyTo ? 'Write a reply…' : 'Add a comment…'} placeholderTextColor={COLORS.textTer} style={s.input} multiline />
                    <TouchableOpacity onPress={submitComment} disabled={sending || !text.trim()} style={[s.sendBtn, (!text.trim() || sending) && { opacity: 0.35 }]} activeOpacity={0.7}>{sending ? <ActivityIndicator size="small" color={COLORS.textInverse} /> : <Ionicons name="send" size={15} color={COLORS.textInverse} />}</TouchableOpacity>
                  </View>
              </View>
            )}
          </Animated.View>
...
const s = StyleSheet.create({
  ...
  header: { paddingTop: Platform.OS === 'ios' ? 58 : 42, paddingBottom: SPACE.md, paddingHorizontal: SPACE.lg, flexDirection: 'row', alignItems: 'center', gap: SPACE.md, backgroundColor: COLORS.bg, borderBottomWidth: 1, borderBottomColor: 'transparent', zIndex: 10 },
  headerIcon: { width: 40, height: 40, borderRadius: RADIUS.full, backgroundColor: COLORS.surface, alignItems: 'center', justifyContent: 'center', ...SHADOW.sm },
  ...
  card: { backgroundColor: COLORS.surface, borderRadius: RADIUS.xl, overflow: 'hidden', borderWidth: 1, borderColor: COLORS.borderLight, ...SHADOW.md },
  ...
  title: { fontSize: FONT.md + 2, fontWeight: FONT.w8, color: COLORS.text, paddingHorizontal: SPACE.lg, marginTop: SPACE.md, lineHeight: 22, letterSpacing: -0.1 },
  body:  { fontSize: FONT.sm, color: COLORS.textSec, lineHeight: 20 },
  readMoreTxt: { fontSize: FONT.xs, color: COLORS.brand, fontWeight: FONT.w7, marginTop: 4 },
  ...
  imgWrap: { marginTop: SPACE.md, marginHorizontal: SPACE.lg, borderRadius: RADIUS.lg, overflow: 'hidden', backgroundColor: COLORS.bgAlt },
  img:     { width: '100%', height: 180 },
  videoPreviewWrap: { position: 'relative', width: '100%', height: 180 },
  playOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.35)' },
  ...
  pill: { flexDirection: 'row', alignItems: 'center', gap: SPACE.xs + 1, borderWidth: 1.5, borderColor: COLORS.border, paddingHorizontal: SPACE.md, paddingVertical: SPACE.sm, borderRadius: RADIUS.full, backgroundColor: COLORS.surface },
  ...
  moreBtn: { width: 36, height: 36, borderRadius: RADIUS.full, borderWidth: 1.5, borderColor: COLORS.border, alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.surface },
  ...
  sheet: { backgroundColor: COLORS.surface, borderTopLeftRadius: RADIUS.xxl, borderTopRightRadius: RADIUS.xxl, maxHeight: H * 0.88, minHeight: H * 0.5, ...SHADOW.xl },
  ...
  cReplyCard: { marginTop: SPACE.sm, marginLeft: SPACE.xxxl + SPACE.sm, flexDirection: 'row', gap: SPACE.sm, paddingHorizontal: SPACE.md, paddingVertical: SPACE.sm, backgroundColor: COLORS.bgAlt, borderRadius: RADIUS.md, borderLeftWidth: 3, borderLeftColor: COLORS.brandLight },
  ...
  inputBar: { position: 'absolute', left: 0, right: 0, bottom: 0, paddingHorizontal: SPACE.md, paddingTop: SPACE.md, paddingBottom: Platform.OS === 'ios' ? SPACE.xxxl : SPACE.md, borderTopWidth: 1, borderTopColor: COLORS.borderLight, backgroundColor: COLORS.surface, ...SHADOW.md },
  ...
  input: { flex: 1, minHeight: 44, maxHeight: 100, borderWidth: 1.5, borderColor: COLORS.border, borderRadius: RADIUS.xl, paddingHorizontal: SPACE.lg, paddingVertical: SPACE.sm + 2, fontSize: FONT.sm, color: COLORS.text, backgroundColor: COLORS.bg },
  sendBtn: { width: 44, height: 44, borderRadius: RADIUS.full, alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.brand, ...SHADOW.brand },
  mediaViewerContainer: { flex: 1, backgroundColor: 'rgba(0,0,0,0.92)', justifyContent: 'center' },
  mediaCloseBtn: { position: 'absolute', top: Platform.OS === 'ios' ? 60 : 40, right: 20, zIndex: 100, padding: 10 },
});
